...
const PROMPT_CONSTRAINTS: &str = "\
             ...
             - Before choosing project-specific paths, naming, file layout, or localisation, call list_agent_preferences with mod_root when the current HOI4 mod root is known. Treat preferences as the effective view, and preserve global_preferences, mod_preferences, and each item's provenance so a mod override applies only to that mod.\n\
             - Store genuinely cross-project habits in the global scope by omitting mod_root. Store project conventions with that mod's root, and never promote one project's paths or naming rules into a user-global habit without clear user intent.\n\
             - For RNMDB state integrity or backup work, call inspect_rhoiscribe_state first and use deep_verify=true when authenticated page verification is needed. Call backup_rhoiscribe_state as a dry run before apply=true, always use an explicit new destination, and never expect overwrite behavior.\n\
             - Do not write SQL, expose keys, request automatic restore, overwrite an existing backup, or guess repairs for the RHoiScribe state database. MCP and SKILL use the same catalog and execution engine for scoped preferences and maintenance results.\n\
             - When a workspace root is known, call discover_hoi4_environment first if the HOI4 game root is not already known. Pass the discovered game_path as vanilla_root to open_hoi4_language_workspace, use mode=full when vanilla-aware CWT context is useful and mode=mod_only for fast mod-only checks, then poll get_hoi4_language_status so CWT rules, diagnostics, symbols, references, completions, and localisation overlays are warm in process memory before broad edits.\n\
             - For ordinary chat snippets with no workspace or saved file, do not open a workspace and do not invent disk files. Call validate_hoi4_file with content and a plausible mod-relative virtual path; if the path is unknown, omit path and let RHoiScribe choose an in-memory conversation path.\n\
             - CWT language support is process-memory only and creates no persistent tool logs. Do not create extracted rule folders, CWT cache files, lock files, snapshots, or RNMDB-backed CWT state; use the embedded CWT catalog and metadata resources for traceability.\n\
